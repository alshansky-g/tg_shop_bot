...
async def test_add_banner_description(session):
    await orm_add_banner_description(session, {'Главная': 'Добро пожаловать'})
    banner = await orm_get_banner(session, 'Главная')
    assert banner is not None
    assert banner.description == 'Добро пожаловать'


async def test_add_banner_description_idempotent(session):
    await orm_add_banner_description(session, {'Главная': 'Первый раз'})
    await orm_add_banner_description(session, {'Главная': 'Второй раз'})  # не должно перезаписать
    banner = await orm_get_banner(session, 'Главная')
    assert banner.description == 'Первый раз'


async def test_change_banner_image(session):
    await orm_add_banner_description(session, {'Оплата': 'Описание'})
    await orm_change_banner_image(session, 'Оплата', 'new_image.jpg')
    banner = await orm_get_banner(session, 'Оплата')
    assert banner.image == 'new_image.jpg'


async def test_get_banner_not_found(session):
    banner = await orm_get_banner(session, 'Несуществующий')
    assert banner is None


async def test_get_info_pages(session):
    await orm_add_banner_description(session, {'А': 'Текст А', 'Б': 'Текст Б'})
...
async def test_seed_products_fills_db(session):
    await orm_create_categories(session, ['Кат 1'])
    categories = await orm_get_categories(session)
    cat_id = categories[0].id

    await orm_seed_products(
        session,
        [('Товар', 'Описание', 100.0, 'Кат 1')],
        placeholder_image='img.png',
    )
    products = await orm_get_products(session, cat_id)
    assert len(products) == 1
    assert products[0].name == 'Товар'


async def test_seed_products_idempotent(session):
    await orm_create_categories(session, ['Кат 1'])
    categories = await orm_get_categories(session)
    cat_id = categories[0].id

    await orm_seed_products(session, [('Товар', 'Описание', 100.0, 'Кат 1')], 'img.png')
    await orm_seed_products(session, [('Другой', 'Описание', 200.0, 'Кат 1')], 'img.png')

    products = await orm_get_products(session, cat_id)
    assert len(products) == 1  # второй вызов не добавил ничего


async def test_seed_skips_unknown_category(session):
    await orm_create_categories(session, ['Кат 1'])
    categories = await orm_get_categories(session)
    cat_id = categories[0].id
    ...
    await orm_seed_products(
        session,
        [('Товар', 'Описание', 100.0, 'Несуществующая категория')],
        'img.png',
    )
    products = await orm_get_products(session, cat_id)
