...
- Оплата заказа через Telegram Payments (ЮКасса)
...
- Команда `/admin` в группе доступна только владельцу (`OWNER_ID`) — подтягивает актуальных администраторов чата и выдаёт им права управления ботом
...
| `OWNER_ID` | Telegram user_id владельца — только он может выдавать права через `/admin` | `123456789` |
| `PAYMENT_TOKEN` | Токен платёжного провайдера от [@BotFather](https://t.me/BotFather) (ЮКасса и др.) | |
...
└── bot/
    ...
    │   └── payments.py           # Telegram Payments: инвойс, pre_checkout, успешная оплата
    ├── keyboards/
